...
/**
 * Busca produtos de uma loja específica via Shopee Open API GraphQL (shopOfferV2).
 * Aceita shopId (Int). Usado pra enriquecer um produto específico dado seu URL —
 * extraímos shopId da URL, listamos produtos da loja, filtramos pelo itemId.
 *
 * FREE — não precisa Apify. Cobre o long tail que productOfferV2(keyword) perde.
 */
export async function fetchShopeeByShopId(
  shopId: number,
  opts: { limit?: number; sortType?: number } = {},
): Promise<RawOffer[]> {
  const limit = opts.limit ?? 50;
  const sortType = opts.sortType ?? 2;
  const query = `
    query ShopOfferV2($shopId: Int!, $limit: Int, $sortType: Int) {
      shopOfferV2(shopId: $shopId, limit: $limit, sortType: $sortType) {
        nodes {
          itemId
          productName
          commissionRate
  ...
  type Resp = { shopOfferV2: { nodes: Node[] } };
  const data = await gql<Resp>(query, { shopId, limit, sortType });
  ...
  return nodes.map<RawOffer>((n) => {
    ...
    return {
      ...
      salesCount: n.sales,
      category: n.productCatIds?.[0] ? String(n.productCatIds[0]) : undefined,
      raw: { ...n, seller: n.shopName } as Record<string, unknown>,
    };
  });
}
